...
select_video_files() {
    ...
    for attempt in {1..3}; do
        selected_files=$(zenity --file-selection \
            ...
            --multiple \
            --file-filter='(mp4, mkv, mov, avi, flv, mpg) | *.mp4 *.mkv *.mov *.avi *.flv *.mpg' \
            --separator="|" \
            2>/dev/null)
    ...
    if [ -z "$selected_files" ]; then
        zenity --error \
            ...
            --text="Could not properly select video files after multiple attempts.\nPlease try running the script again." \
            --width=300
        exit 1
    fi
...
needs_transcoding() {
    ...
    # Determine the output file path following the _edit_friendly_format.mov naming convention
    if [[ "$input_file" == *_edit_friendly_format.mov ]]; then
        # If input is already a _edit_friendly_format.mov file, use the same name
        output_file="${output_folder}/$(basename "$input_file")"
    else
        # For other files, append _edit_friendly_format.mov to the base name
        output_file="${output_folder}/$(basename "${input_file%.*}")_edit_friendly_format.mov"
    fi

    # Case 1: Input file already has _edit_friendly_format.mov extension
    if [[ "$input_file" == *_edit_friendly_format.mov ]]; then
        echo "# Skipping: Input file is already in _edit_friendly_format.mov format" >&2
...
transcode_files() {
    ...
    local video_codec="dnxhr_hq" audio_codec="pcm_s24le"
    ...
    for file in "${input_files[@]}"; do
        ...
        output_file="${output_folder}/${filename%.*}_edit_friendly_format.mov"

        mkdir -p "$output_folder"

        echo "# Transcoding video file: $filename ($current_file of $total_files)"
        ...
        if needs_transcoding "$file" "$video_codec"; then
            if ffmpeg -hide_banner -loglevel warning -i "$file" \
                -c:v dnxhd -profile:v "$video_codec" -c:a "$audio_codec" -threads 0 \
                "$output_file" -y 2>/dev/null; then
                echo "${filename%.*}_edit_friendly_format.mov" >> "$success_tmpfile"
            else
        ...
        --title="Transcoding Video File(s)" \
...
main() {
    ...
    local input_files=$(select_video_files)
